...
static struct menu_item *menu_item_add(struct menu *menu, struct menu *submenu, const char *name, int id, D3DCOLOR color, void *data)
{
...
static void menu_cheats_mods_populate(struct menu *menu)
{
	...
	if (vinfo->passengers[0] != ainfo) return;
	// some variables
	...
	*/
	...
	for (int iUpgradeInfo = 0; iUpgradeInfo < VEHICLE_UPGRADES_COUNT; iUpgradeInfo++)
	{
		...
		{
			snprintf(name, sizeof(name), "%s %s", cveh_upgrade_info[iUpgradeInfo].upgradeCategoryName, cveh_upgrade_info[iUpgradeInfo].upgradeName);
			...
			if (cveh_upgrade_info[iUpgradeInfo].upgradeCategoryName == "Sideskirt"
				|| cveh_upgrade_info[iUpgradeInfo].upgradeCategoryName == "Vents")
				iUpgradeInfo++;
		}
	}
...
static void menu_vehicles_populate(struct menu *menu, int class_id)
{
   ...
   if(pool_vehicle != NULL)
   {
      for(i=0; i<pool_vehicle->size; i++)
      {
         ...
         snprintf(name, sizeof(name), "%s (%.2fm%s)",
               vehicle->name,
               vect3_length(dist),
...
static int menu_callback_telepickup(int op, struct menu_item *item)
{
	if(op == MENU_OP_SELECT)
	{
		...
		if(g_SAMP->pPool_Pickup == NULL) return 0;
		if(g_SAMP->pPool_Pickup->pickup[id].iType == 0) return 0;
		if(g_SAMP->pPool_Pickup->pickup[id].iModelID == 0) return 0;
		if(item->id == ID_NONE) return 0;
...
static void menu_teleobject_populate(struct menu *menu)
{
	...
	for(i = 0; i < SAMP_OBJECTS_MAX; i++)
	{
		...
		snprintf(text, sizeof(text), "Object (%d, ModelID %d)", i, g_SAMP->pPool_Object->object[i]->pGTAObject->base.model_alt_id);
		menu_item_add(menu, NULL, text, i, color, NULL);
	}
}
...
static int menu_callback_teleobject(int op, struct menu_item *item)
{
	if(op == MENU_OP_SELECT)
	{
		if(g_SAMP->pPool_Object == NULL) return 0;
		if(item->id == ID_NONE) return 0;
...
static int menu_callback_cheats_mods(int op, struct menu_item *item)
{
	...
	case MENU_OP_INC:
		...
		case ID_CHEAT_MODS_PAINTJOB:
			...
			mod = (op == MENU_OP_DEC) ? - 1: 1;
			if (mod > 0) paintjob_temp++;
			if (mod < 0) paintjob_temp--;
			if (paintjob_temp < 0) paintjob_temp = 0;
			if (paintjob_temp > 4) paintjob_temp = 4;
	...
	case MENU_OP_SELECT:
		...
		{
			struct actor_info *ainfo = actor_info_get(ACTOR_SELF, 0);
			if (vinfo->passengers[0] != ainfo) return 0;
			...
			if (upgrade_info.upgradeCategoryName == "Sideskirt"
				|| upgrade_info.upgradeCategoryName == "Vents")
			{
...
static int menu_callback_cheats(int op, struct menu_item *item)
{
   ...
   case MENU_OP_ENABLED:
      switch(item->id)
      {
      case ID_CHEAT_INVULN:			return cheat_state->_generic.hp_cheat;
      case ID_CHEAT_WEAPONS:		return 0;
      case ID_CHEAT_MONEY:			return cheat_state->_generic.money;
	  case ID_CHEAT_MODS:			return 0;
      case ID_CHEAT_PROT:			return cheat_state->vehicle.protection;
      case ID_CHEAT_WARP_NEAR:		return 0;
      case ID_CHEAT_JETPACK:		return 0;
      case ID_CHEAT_UNLOCK:			return cheat_state->_generic.vehicles_unlock;
      case ID_CHEAT_WEATHER:		return (set.force_weather == (int)(UINT_PTR)item->data);
      case ID_CHEAT_TIME:			return (set.force_hour    == (int)(UINT_PTR)item->data);
      case ID_CHEAT_HP:				return 0;
      case ID_CHEAT_ARMOR:			return 0;
      case ID_CHEAT_GRAVITY:		return 0;
...
static int menu_callback_handling(int op, struct menu_item *item)
{
	...
	case MENU_OP_INC:
		...
		case ID_CHEAT_HANDLING_MULTIPLIER:
			if(mod > 0) set.handling_multiplier += 0.050f;
			if(mod < 0) set.handling_multiplier -= 0.050f;
			menu_item_name_set(item, "Multiplier: %0.2f", set.handling_multiplier);
			return 1;

		case ID_CHEAT_HANDLING_ENGNACCEL:
			if(mod > 0) info->pHandlingData->Transmission.fEngineAccelleration += set.handling_multiplier;
			if(mod < 0) info->pHandlingData->Transmission.fEngineAccelleration -= set.handling_multiplier;
			menu_item_name_set(item, "Engine acceleration: %0.2f", info->pHandlingData->Transmission.fEngineAccelleration);
			return 1;

		case ID_CHEAT_HANDLING_BRAKEDECEL:
			if(mod > 0) info->pHandlingData->fBrakeDecelleration += set.handling_multiplier;
			if(mod < 0) info->pHandlingData->fBrakeDecelleration -= set.handling_multiplier;
			menu_item_name_set(item, "Brake decelleration: %0.2f", info->pHandlingData->fBrakeDecelleration);
			return 1;

		case ID_CHEAT_HANDLING_TRACMULT:
			if(mod > 0) info->pHandlingData->fTractionMultiplier += set.handling_multiplier;
			if(mod < 0) info->pHandlingData->fTractionMultiplier -= set.handling_multiplier;
		...
		case ID_CHEAT_HANDLING_TRACTIONLOSS:
			if(mod > 0) info->pHandlingData->fTractionLoss += set.handling_multiplier;
			if(mod < 0) info->pHandlingData->fTractionLoss -= set.handling_multiplier;
		...
		case ID_CHEAT_HANDLING_SUSPFORCELVL:
			if(mod > 0) info->pHandlingData->fSuspensionForceLevel += set.handling_multiplier;
			if(mod < 0) info->pHandlingData->fSuspensionForceLevel -= set.handling_multiplier;
...
static int menu_callback_cheats_invuln(int op, struct menu_item *item)
{
   ...
   case MENU_OP_ENABLED:
      switch(item->id)
      {
      case ID_CHEAT_INVULN_ACTOR:		   return cheat_state->actor.invulnerable;
      case ID_CHEAT_INVULN_VEHICLE:		   return cheat_state->vehicle.invulnerable;
      case ID_CHEAT_INVULN_TIRES:		   return cheat_state->vehicle.hp_tire_support;
      case ID_CHEAT_INVULN_MIN_HP:		   return cheat_state->vehicle.hp_minimum_on;
      case ID_CHEAT_INVULN_DMGRED:		   return cheat_state->vehicle.hp_damage_reduce_on;
      case ID_CHEAT_INVULN_REGEN:		   return cheat_state->vehicle.hp_regen_on;
      case ID_CHEAT_INVULN_REGEN_ONFOOT:   return cheat_state->actor.hp_regen_on;
      }
      break;
   ...
   case MENU_OP_SELECT:
      switch(item->id)
      {
      case ID_CHEAT_INVULN_ACTOR:		   cheat_state->actor.invulnerable ^= 1;          break;
      case ID_CHEAT_INVULN_VEHICLE:		   cheat_state->vehicle.invulnerable ^= 1;        break;
      case ID_CHEAT_INVULN_TIRES:		   cheat_state->vehicle.hp_tire_support ^= 1;     break;
      case ID_CHEAT_INVULN_MIN_HP:		   cheat_state->vehicle.hp_minimum_on ^= 1;       break;
      case ID_CHEAT_INVULN_DMGRED:		   cheat_state->vehicle.hp_damage_reduce_on ^= 1; break;
      case ID_CHEAT_INVULN_REGEN:		   cheat_state->vehicle.hp_regen_on ^= 1;         break;
      case ID_CHEAT_INVULN_REGEN_ONFOOT:   cheat_state->actor.hp_regen_on ^= 1;           break;
      default:
         return 0;
      }
      return 1;
...
static int menu_callback_cheats_money(int op, struct menu_item *item)
{
   ...
   case MENU_OP_SELECT:
      ...
      switch(item->id)
      {
      case ID_CHEAT_MONEY_GIVE_500:    money += 500;   break;
      case ID_CHEAT_MONEY_GIVE_1000:   money += 1000;  break;
      case ID_CHEAT_MONEY_GIVE_5000:   money += 5000;  break;
      case ID_CHEAT_MONEY_GIVE_10000:  money += 10000; break;
      case ID_CHEAT_MONEY_GIVE_20000:  money += 20000; break;
	  case ID_CHEAT_MONEY_GIVE_99999999: money += 99999999; break;
	  case ID_CHEAT_MONEY_GIVE_CUSTOM: money += set.money_value; break;
      }
      gta_money_set(money);
      break;
   ...
   case MENU_OP_INC:
      ...
      switch(item->id)
      {
      case ID_CHEAT_MONEY_MAX:      set.money_amount_max += 50000 * mod; break;
      case ID_CHEAT_MONEY_RAND_MIN: set.money_amount_rand_min += 500 * mod; break;
      case ID_CHEAT_MONEY_RAND_MAX: set.money_amount_rand_max += 500 * mod; break;
      case ID_CHEAT_MONEY_IVAL_MIN: set.money_interval_rand_min += 1 * mod; break;
      case ID_CHEAT_MONEY_IVAL_MAX: set.money_interval_rand_max += 1 * mod; break;
      }
      switch(item->id)
      {
      case ID_CHEAT_MONEY_MAX:      menu_item_name_set(item, "Max money: %d", (int)set.money_amount_max);              break;
      case ID_CHEAT_MONEY_RAND_MIN: menu_item_name_set(item, "Random min: %d", (int)set.money_amount_rand_min);        break;
      case ID_CHEAT_MONEY_RAND_MAX: menu_item_name_set(item, "Random max: %d", (int)set.money_amount_rand_max);        break;
      case ID_CHEAT_MONEY_IVAL_MIN: menu_item_name_set(item, "Interval min: %dsec", (int)set.money_interval_rand_min); break;
      case ID_CHEAT_MONEY_IVAL_MAX: menu_item_name_set(item, "Interval max: %dsec", (int)set.money_interval_rand_max); break;
      default:
         return 0;
      }
   }
...
static int menu_callback_weapons(int op, struct menu_item *item)
{
   ...
   if(op == MENU_OP_ENABLED)
   {
      ...
      case ID_WEAPON_ENABLE: return cheat_state->_generic.weapon;
	  case ID_WEAPON_RANDOM: return set.ammo_random;
      }
...
static int menu_callback_vehicles(int op, struct menu_item *item)
{
   if(op == MENU_OP_ENABLED)
   {
      switch(item->id)
      {
      case ID_VEHICLES_IWARP:  return cheat_state->_generic.vehicles_warp_invert;
      case ID_VEHICLES_FREEZE: return cheat_state->_generic.vehicles_freeze;
      }
   }
   else if(op == MENU_OP_SELECT)
   {
      switch(item->id)
      {
      case ID_VEHICLES_IWARP:  cheat_state->_generic.vehicles_warp_invert ^= 1; break;
      case ID_VEHICLES_FREEZE: cheat_state->_generic.vehicles_freeze ^= 1;      break;
      default:
         return 0;
      }
      return 1;
...
static int menu_callback_vehicles_sub(int op, struct menu_item *item)
{
   if(op == MENU_OP_SELECT && item->id != ID_NONE)
   {
      ...
      struct vehicle_info *info = vehicle_info_get(item->id,
...
static int menu_callback_misc(int op, struct menu_item *item)
{
	...
	if(op == MENU_OP_ENABLED)
	{
      switch(item->id)
      {
      case ID_MISC_HUDTEXT:				return set.d3dtext_hud;
      case ID_MISC_BAD_WEAPONS:			return cheat_state->_generic.weapon_disable;
      case ID_MISC_TRAILERS:			return set.trailer_support;
	  case ID_MISC_FPSLIMIT:			return 0;
	  case ID_MISC_TOGGLEWINDOWED:		return set.window_mode;
	  case ID_MISC_CLEANSCREENSHOT:		return set.screenshot_clean;
      }
   }
   else if(op == MENU_OP_SELECT)
   {
      ...
      case ID_MISC_COORDS:
         {
            ...
            Log("static_teleport_pos[] = %.2f %.2f %.2f   %d",
	...
	case MENU_OP_ENABLED:
		{
			switch(item->id)
			{
			case ID_MENU_SAMPMISC_FAKE_KILL: return 0;
			case ID_MENU_SAMPMISC_SPOOF_WEAPON: return (cheat_state->_generic.spoof_weapon == (int)(UINT_PTR)item->data);
			case ID_MENU_SAMPMISC_SAMP_INFO_LIST: return cheat_state->player_info_list;
			case ID_MENU_SAMPMISC_CHAT_TEXT: return (g_Chat == NULL) ? 0 : !g_Chat->iChatWindowMode;
			case ID_MENU_SAMPMISC_CHAT_TEXTLINES: return 0;
			case ID_MENU_SAMPMISC_SAMP_CJ_ANIM: return g_SAMP->bytePlayerPedAnims;
			case ID_MENU_SAMPMISC_RENDEROBJTXT: return cheat_state->_generic.objecttexts;
			case ID_MENU_SAMPMISC_RENDERPCKTXT: return cheat_state->_generic.pickuptexts;
			}
		}
	case MENU_OP_SELECT:
		{
			...
			case ID_MENU_SAMPMISC_FAKE_KILL:
				{
					...
					if(g_Players->pRemotePlayer[(int)(UINT_PTR)item->data] == NULL) return 0;
			...
			case ID_MENU_SAMPMISC_RENDEROBJTXT: cheat_state->_generic.objecttexts ^= 1; break;
			case ID_MENU_SAMPMISC_RENDERPCKTXT: cheat_state->_generic.pickuptexts ^= 1; break;
			}
		}
	case MENU_OP_DEC:
	case MENU_OP_INC:
...
static int menu_callback_players_vehwarp(int op, struct menu_item *item)
{
   if(op == MENU_OP_SELECT)
   {
	  ...
	  if(actor->vehicle == NULL) return 0;
	  ...
	  {
		  vehicleJumper(getPlayerSAMPVehicleID(id));
	  } else {
...
{
	if(g_Players->pLocalPlayer == NULL) return 0;
	...
	if(op == MENU_OP_ENABLED)
	{
		switch(id)
		{
		case ID_MENU_SPECIAL_ACTION_NONE: if(specialaction == ID_MENU_SPECIAL_ACTION_NONE) return 1; break;
		case ID_MENU_SPECIAL_ACTION_USEJETPACK: if(specialaction == ID_MENU_SPECIAL_ACTION_USEJETPACK) return 1; break;
		case ID_MENU_SPECIAL_ACTION_DANCE1: if(specialaction == ID_MENU_SPECIAL_ACTION_DANCE1) return 1; break;
		case ID_MENU_SPECIAL_ACTION_DANCE2: if(specialaction == ID_MENU_SPECIAL_ACTION_DANCE2) return 1; break;
		case ID_MENU_SPECIAL_ACTION_DANCE3: if(specialaction == ID_MENU_SPECIAL_ACTION_DANCE3) return 1; break;
		case ID_MENU_SPECIAL_ACTION_DANCE4: if(specialaction == ID_MENU_SPECIAL_ACTION_DANCE4) return 1; break;
		case ID_MENU_SPECIAL_ACTION_HANDSUP: if(specialaction == ID_MENU_SPECIAL_ACTION_HANDSUP) return 1; break;
		case ID_MENU_SPECIAL_ACTION_USECELLPHONE: if(specialaction == ID_MENU_SPECIAL_ACTION_USECELLPHONE) return 1; break;
		case ID_MENU_SPECIAL_ACTION_STOPUSECELLPHONE: if(specialaction == ID_MENU_SPECIAL_ACTION_STOPUSECELLPHONE) return 1; break;
		case ID_MENU_SPECIAL_ACTION_DRINK_BEER: if(specialaction == ID_MENU_SPECIAL_ACTION_DRINK_BEER) return 1; break;
...
static int menu_callback_debug(int op, struct menu_item *item)
{
   ...
   if(op == MENU_OP_SELECT)
   {
	  ...
	  case ID_DEBUG_SELF_ACTOR:		    setDebugPointer((void *)actor_info_get(ACTOR_SELF, 0)); break;
      case ID_DEBUG_SELF_VEHICLE:		setDebugPointer((void *)vehicle_info_get(VEHICLE_SELF, 0)); break;
	  case ID_DEBUG_SAMP_DLL:			setDebugPointer((void *)g_dwSAMP_Addr); break;
      case ID_DEBUG_SAMP_INFO:			setDebugPointer((void *)g_SAMP); break;
	  case ID_DEBUG_SAMP_PLAYER_LIST:   setDebugPointer((void *)g_Players); break;
	  case ID_DEBUG_SAMP_LOCAL_INFO:	setDebugPointer((void *)g_Players->pLocalPlayer); break;
	  case ID_DEBUG_SAMP_CHAT_INFO:     setDebugPointer((void *)g_Chat); break;
	  case ID_DEBUG_SAMP_CHAT_IPT_INFO: setDebugPointer((void *)g_Input); break;
	  case ID_DEBUG_SAMP_KILL_INFO:     setDebugPointer((void *)g_DeathList); break;
	  case ID_DEBUG_SAMP_VEHICLE_LIST:  setDebugPointer((void *)g_Vehicles); break;
...
static int menu_callback_hudindicators(int op, struct menu_item *item)
{
   ...
   if(op == MENU_OP_SELECT)
   {
      switch(item->id)
      {
	  case ID_HUDIND_BAR: set.hud_draw_bar ^= 1; break;
	  case ID_HUDIND_TSHADOWS: set.render_text_shadows ^= 1; break;	  
	  case ID_HUDIND_INV: set.hud_indicator_inv ^= 1; break;
	  case ID_HUDIND_WEAPON: set.hud_indicator_weapon ^= 1; break;
	  case ID_HUDIND_MONEY: set.hud_indicator_money ^= 1; break;
	  case ID_HUDIND_FREEZE: set.hud_indicator_freeze ^= 1; break;
	  ...
	  case ID_HUDIND_POS: set.hud_indicator_pos ^= 1; break;
	  case ID_HUDIND_FPS: set.hud_fps_draw ^= 1; break;
	  case ID_HUDIND_LB_BARS: set.left_bottom_bars_enable ^= 1; break;
...
static int menu_callback_server_list(int op, struct menu_item *item)
{
	...
	if(g_SAMP == NULL && g_Players == NULL) return 0;
...
// testing stuff
   ...
   menu_item_add(menu_cheats_money, NULL, name, ID_CHEAT_MONEY_IVAL_MAX, MENU_COLOR_DEFAULT, NULL);
	...
	const struct { int id; char *name; } weather_map[] =
	{
		0, "Blue sky",
		8, "Stormy",
		9, "Cloudy and foggy",
		10, "Clear blue sky",
		11, "Scorching hot (heat waves)",
		12, "Very dull, colorless, hazy",
		...
		23, "Variations of pale orange",
		27, "Variations of fresh blue",
		30, "Variations of dark, cloudy, teal",
	...
	for(i=0; ; i++)
	{
		if(weather_map[i].name == NULL)
			break;
		menu_item_add(menu_cheats_weather, NULL, weather_map[i].name, ID_CHEAT_WEATHER, MENU_COLOR_DEFAULT, (void *)(UINT_PTR)weather_map[i].id);
	}
   ...
   menu_item_add(menu_players, menu_player_info,     "Show infos on player",          ID_MENU_PLAYERS_DEBUG,     MENU_COLOR_DEFAULT, NULL);
#endif
   ...
   /* teleports */
   menu_item_add(menu_teleports, menu_interiors, "Teleport to interiors", STATIC_TELEPORT_MAX + 1, MENU_COLOR_DEFAULT, NULL);
   for(i=0; i<146; i++)
   {
   ...
   for(i=0; weapon_list[i].name!=NULL; i++)
   {
      ...
      menu_item_add(menu_spoof_weapon, NULL, name, ID_MENU_SAMPMISC_SPOOF_WEAPON, MENU_COLOR_DEFAULT, (void *)(UINT_PTR)weapon->id);
   }
   menu_item_add(menu_spoof_weapon, NULL, "Fake death by fall", ID_MENU_SAMPMISC_SPOOF_WEAPON, MENU_COLOR_DEFAULT, (void *)(UINT_PTR)20);
   menu_item_add(menu_spoof_weapon, NULL, "Fake begin run over", ID_MENU_SAMPMISC_SPOOF_WEAPON, MENU_COLOR_DEFAULT, (void *)(UINT_PTR)21);
