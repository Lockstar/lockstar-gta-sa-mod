...
// new function to jump into vehicles without jacking
void vehicleJumper(int iVehicleID)
{
	...
	// safty checks, we should NOT have to do this because this should ALREADY be done
	if(g_SAMP != NULL)
	{
		if(g_Vehicles == NULL || g_Players == NULL) return;
	}
	else return;
	...
	if (!pVehicle->base.bUsesCollision) {
		addMessageToChatWindow("Can't get in a vehicle that doesn't have collisions enabled.");
		return;
	}
	if (!pVehicle->base.bIsVisible)
	{
		addMessageToChatWindow("Vehicle is not visible.");
		return;
	}
	if(actor_info_get(ACTOR_SELF, 0) != NULL &&
		pVehicle->base.interior_id != actor_info_get(ACTOR_SELF, 0)->base.interior_id)
	{
		addMessageToChatWindow("Vehicle is in another interior.");
		return;
	}

	iVehicleID = getSAMPVehicleIDFromGTAVehicle(pVehicle);
	int iGTAVehicleID;
	iGTAVehicleID = getVehicleGTAScriptingIDFromVehicleID(iVehicleID);

	// is someone driving the target vehicle?
	int vehicle_has_driver = 0;
	if (pVehicle->passengers[0] != NULL)
		vehicle_has_driver = 1;

	// put into first available seat
	if (vehicle_has_driver == 0) {
		ScriptCommand(&put_actor_in_car, 1, iGTAVehicleID);
	...
	vehicle = gta_vehicle_get_by_id(pVehicle->base.model_alt_id);
	const int seat_count = vehicle->passengers;
	if (seat_count > 0) {
		for (int seat = 1; seat <= seat_count; seat++) {
			if(pVehicle->passengers[seat] == NULL) {
				ScriptCommand(&put_actor_in_car_passenger, 1, iGTAVehicleID, seat-1);
				ScriptCommand(&restore_camera_with_jumpcut);
				ScriptCommand(&set_camera_directly_behind);
				ScriptCommand(&restore_camera_with_jumpcut);
				return;
	...
	addMessageToChatWindow("No seats left to teleport into.");
	return;
}
