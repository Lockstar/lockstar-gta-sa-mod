...
static void ini_entry_parse_type(struct ini_entry *ent, int idx, const char *val)
{
   ...
   case TYPE_BOOL:
      if(split->argc != 1)
         goto wrong_argc;
      if(strcmp(split->argv[0], "true") == 0 || strcmp(split->argv[0], "True") == 0 || strcmp(split->argv[0], "TRUE") == 0)
         *(int *)ent_data->data = 1;
      else if(strcmp(split->argv[0], "false") == 0 || strcmp(split->argv[0], "False") == 0 || strcmp(split->argv[0], "FALSE") == 0)
         *(int *)ent_data->data = 0;
      else
      {
...
parse_continue:;
   ...
   for(i=0; i<ini_entry_count; i++)
   {
      ...
      for(j=0; j<ent->size; j++)
      {
         ...
         {
            Log("Warning: setting '%s' was set multiple times in the .ini file",
               ini_entry_name(ent, j));
         }
         isset = 1;
      }
      if(!isset)
         Log("Warning: setting '%s%s' was not present in the .ini file",
            ent->name, (ent->size > 1) ? "[]" : "");
   }
