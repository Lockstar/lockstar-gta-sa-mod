...
// hacked to have no member functions that rely on CGame
// cos we're not doing CGame yet.  maybe with a complete rebuild :P
...
public:
	...
	DWORD		*m_pLastRenderedLink;	// 40 // CLink<CEntity*>* m_pLastRenderedLink;
	
	WORD m_nScanCode;					// 44
	BYTE m_iplIndex;					// 46 // used to define which IPL file object is in
	BYTE m_areaCode;					// 47 // used to define what objects are visible at this point
	...
	//********* BEGIN CEntityInfo **********//
	BYTE nType : 3;						// 54 // what type is the entity (2 == Vehicle)
	BYTE nStatus : 5;					// 54 // control status
	//********* END CEntityInfo **********//
	...
	// not really, they commented out the CVehicle pad, but whatevah
};
