...
/// \note This class is not safe to call from multiple threads at the same time.  For a multithreaded app, either limit your RakNet calls to one thread, or limit function calls to one thread and send the output to worker threads.
...
public:
	...
	/// \return The index of this PlayerID or -1 on player not found.
	...
	/// To simulate bi-directional ping and packet loss, you should call this on both the sender and the recipient, with half the total ping and maxPacketsOnWire value on each.
	/// You can exclude network simulator code with the _FINAL #define to decrease code size
	/// \param[in] maxPacketsOnWire The maximum window size for the windowing algorithm before 100% packetloss.  The chance of each packet being lost is RakNetStatisticsStruct::windowSize / maxPacketsOnWire .  Recommended you set this between 100 and 1000 with lower numbers corresponding to greater packetloss.  This approximates the real condition where the more data you send the more likely you are to lose data. Call with 0 to disable.
