...
public:
	...
	void Init(unsigned short speexSampleRate, unsigned bufferSizeBytes, RakPeerInterface *rakPeer);
	...
	bool SendFrame(void *inputBuffer);
	...
	unsigned GetBufferedBytesToSend() const;
	...
	unsigned GetBufferedBytesToReturn(PlayerIndex playerIndex) const;

	void RequestVoiceChannel();
	...
	virtual void OnClosedConnection(PlayerIndex playerIndex, int lostConnectionReason );
protected:
	virtual void OnVoiceData(Packet *packet, PlayerIndex playerIndex);
	void OpenChannel(PlayerIndex playerIndex);
	void OpenLocalChannel();
	void FreeChannelMemory(PlayerIndex playerIndex);
	void FreeChannelMemory(unsigned index, bool removeIndex);
	void FreeLocalChannelMemory();
	...
	VoiceChannel *voiceChannels[SAMP_PLAYER_MAX];
	VoiceChannel *localChannel;
