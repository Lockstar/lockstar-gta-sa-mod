...
//#define _USE_SPSC_MUTEX

static const int MINIMUM_LIST_SIZE=8;

#ifdef _USE_SPSC_MUTEX
#include "SimpleMutex.h"
#define LOCK_MUTEX pointerMutex.Lock();
#define UNLOCK_MUTEX pointerMutex.Unlock();
// Undef volatile
#define SPSC_VOLATILE
#else
// Undef mutex
#define LOCK_MUTEX
#define UNLOCK_MUTEX
#define SPSC_VOLATILE volatile
#endif
...
namespace BasicDataStructures
{
	...
	private:
		struct DataPlusPtr
		{
			...
			SPSC_VOLATILE bool readyToRead;
			SPSC_VOLATILE DataPlusPtr *next;
		};
		SPSC_VOLATILE DataPlusPtr *readAheadPointer;
		SPSC_VOLATILE DataPlusPtr *writeAheadPointer;
		SPSC_VOLATILE DataPlusPtr *readPointer;
		SPSC_VOLATILE DataPlusPtr *writePointer;
		unsigned readCount, writeCount;

#ifdef _USE_SPSC_MUTEX
		SimpleMutex pointerMutex;
#endif
	};
		...
		SingleProducerConsumerType* SingleProducerConsumer<SingleProducerConsumerType>::WriteLock( void )
	{
		LOCK_MUTEX
			...
			{
				SPSC_VOLATILE DataPlusPtr *originalNext=writeAheadPointer->next;
				writeAheadPointer->next=new DataPlusPtr;
				writeAheadPointer->next->next=originalNext;
			}

			SPSC_VOLATILE DataPlusPtr *last;
